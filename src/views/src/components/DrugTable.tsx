...
const DrugTable: React.FC = () => {
  ...
  const highlightStyle = {
    backgroundColor: 'rgba(0, 123, 255, 0.2)', // Light blue highlight
  };
  ...
  const handleCellMouseDown = (value: any, columnId: string) => {
    if (value && value !== "N/A") {
      setDragValue(value);
  ...
  const handleCellMouseEnter = (row: MRT_Row<any>) => {
    if (isDragging && dragValue && dragColumnId) {
      ...
      setTableData((prevData) =>
        prevData.map((drug) => (drug.DrugID === row.original.DrugID ? { ...drug, [dragColumnId]: dragValue } : drug))
      );
      // Highlight the cell
      const cell = document.querySelector(`[data-row-id="${row.id}"][data-column-id="${dragColumnId}"]`);
      if (cell) {
  ...
  const table = useMantineReactTable({
    ...
    mantineTableBodyCellProps: ({ cell }) => ({
      onMouseDown: () => handleCellMouseDown(cell.getValue<any>(), cell.column.id),
      ...
      className: isDragging ? "cell-highlight" : "",
    }),
  });
  ...
  return (
    ...
    >
      <div style={{ marginBottom: "10px", display: "flex", alignItems: "center" }}>
        <label htmlFor="columnPresetSelect" style={{ marginRight: "10px", fontWeight: "bold", fontSize: "14px" }}>
      ...
      <div style={{ marginBottom: "10px", fontSize: "12px", color: "#666" }}>
        <i>Tip: Click and drag a cell value to fill other cells with the same value (Excel-like functionality).</i>
      </div>
