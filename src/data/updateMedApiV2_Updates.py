...
def main():
    ...
    # Get input file
    file_path = input("\nEnter file path (or press Enter for './June30.csv'): ").strip()
    if not file_path:
        file_path = './June30.csv'
    ...
    try:
        ...
        # Get only the MoPHCodes that exist in the file
        print("\n📊 Fetching current drug data for drugs in file...")
        file_moph_codes = list(file_data.keys())
        ...
        cursor.execute(query, file_moph_codes)
        current_data = {row['MoPHCode']: row for row in cursor.fetchall()}
        print(f"✅ Fetched {len(current_data)} matching records from database")
        ...
        # Process drugs that exist in both file and database
        print("🔍 Comparing data and detecting changes...")
        ...
        print(f"\n📊 Total drug updates: {len(update_drug_list)}")
        ...
        # Confirmation
        if not update_drug_list:
            print("\n✅ No changes to commit. All drugs are up to date!")
            return
